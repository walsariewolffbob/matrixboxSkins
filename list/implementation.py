    ...
    def render(self, prepared_data):
        ...
        try:
            for record in self.f.train_data:
                ...
                try:
                    _display_height = int(self.f.display_height)
                except Exception:
                    _display_height = 32
                if _display_height >= 64 and not half and not self.f.rotated:
                    # 2X has enough vertical space for eight small-font rows.
                    # Keep the normal 32px List limits unchanged elsewhere.
                    max_rows = min(8, max(1, int(self.f.settings.get('maxdest', 8))))
                else:
